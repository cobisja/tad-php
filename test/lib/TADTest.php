...
class TADTest extends \PHPUnit_Framework_TestCase
{
  ...
  {
    $xml = '<user><name>foo</name><lastname>bar</lastname><age>99</age></user>';
    $expected_json = '{"name":"foo","lastname":"bar","age":"99"}';
    $json = TAD::xml_to_json($xml);
  ...
  {
    $xml = '<user><name>foo</name><lastname>bar</lastname><age>99</age></user>';
    $expected_array = ['name'=>'foo', 'lastname'=>'bar', 'age'=>'99'];
    $array = TAD::xml_to_array($xml);
  ...
  {
    $expected_xml = '<root><0>foo</0><1>bar</1><2>baz</2></root>';
    $xml = TAD::array_to_xml(["foo", "bar", "baz"]);
  ...
  public function testTAD()
  {
    ...
    $tad->expects( $this->any() )
      ...
      ->will( $this->returnValue('Executed via SOAP') );
    
    $tad->expects( $this->once() )
      ->method('execute_command_via_zklib')
      ->will( $this->returnValue('Executed via ZKLib') );
    
    $this->assertEquals( 'Executed via SOAP',  $tad->get_date() );
    $this->assertEquals( 'Executed via ZKLib', $tad->set_date() );
  }
  ...
  {
    $options = $this->get_tad_and_soap_options();
    $mock_response = '<SetDateResponse><Result>1</Result><Information>Succesfully!</Information></SetDateResponse>';
